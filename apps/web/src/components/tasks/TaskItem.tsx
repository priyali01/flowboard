...
export const TaskItem = ({ task, onToggle, onClick }: TaskItemProps) => {
  ...
  const priorityConfig = {
    ...
    MEDIUM: { color: 'text-amber-500', bg: 'bg-amber-100 dark:bg-amber-900/30', label: 'Medium' },
  ...
  return (
    <div 
      ...
      className={cn(
        "group flex items-center py-4 pr-4 pl-3 bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl border-b border-gray-100 dark:border-gray-700/50 hover:bg-white dark:hover:bg-gray-800 transition-all cursor-pointer relative",
    ...
    >
      ...
      <div 
        {...attributes}
        {...listeners}
        className="text-gray-300 dark:text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing px-1 mr-2 focus:outline-none"
      >
